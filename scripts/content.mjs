...
function renderServicePage(content, service, resource) {
  ...
  const contentHtml = `<div class="container service-page-content">${breadcrumb([{ label: 'Ana Sayfa', href: 'index.html' }, { label: 'Hizmetler', href: 'hizmetler.html' }, { label: service.title }])}
    ...
    ${trainingSection}${sectorSection}${siblingSection}
    ...
    <section class="content-section"><div class="section-heading"><p class="eyebrow">Sık sorulanlar</p><h2>Kapsamı doğru beklentiyle başlatın</h2></div><div class="sector-faq-list">${faq}</div></section>${trustStrip()}
